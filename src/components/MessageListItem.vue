  ...
  <li class="flex flex-col">
    <div class="flex flex-row items-end" :class="{hidden: !hasReply}">
      ...
      <div class="mb-0.5 truncate">
        <img :src="userStore.users[replyUserId]?.profilePicture" alt="profile picture"
        ...
        <span class="hover:underline hover:text-white hover:cursor-pointer"
          ...
          <span class="text-sm mr-0.5">{{ userStore.users[replyUserId]?.userName }}</span>
          <span class="text-xs">{{ userStore.messages[replyId]?.messageText }}</span>
        </span>
      </div>
    </div>
    <div class="flex flex-row justify-between border border-black">
      ...
      <div class="flex flex-col grow w-12">
        <div class="flex items-center gap-1">
          <h3 class="text-lg block truncate">{{ userStore.users[props.message.senderId].userName }}</h3>
